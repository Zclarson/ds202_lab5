---
title: "Team.Rmd"
author: "Julia Lundstrum and Zack Larson"
...
#The state and state code are the same in both dfs and then we added the state name column after the region column in the second df
dangmap <- dmake %>%
...
#We plotted the text labels based on the average coordinates that the accident took place so that will make it more likley to see the text the best
names <- dangmap %>% 
...
The most dangerous vehicle make is being described as the vehicle make that was responsible for the most fatalities in these data sets. This fails to take into account the popularity of the vehicle make.
...
```{r}
#We figured it was easiest to use the hour data for the x axis, the sex for the color since there are only 2 of them, and the day of the week in a facet grid because 7 of those are more appealing than 24 if we used hour
nums <- joined %>%
...
```{r}
#Like we did with the state data, we took the county code and COUNTY number from each data frame and joined it on that. Then we decided it would be best to also include that county name based on the subregion column
